...
def get_tokenized_data(data: pd.DataFrame) -> pd.DataFrame:
    ...
    en_stop = set(nltk.corpus.stopwords.words(
        'english'))  # common english stop words

    # this part gets ran to get the words that are commonly seen together
    # learning of word representation and sentence classification
    pretrained_vectors = FastText(language='en')
    # defines vocab object that will be used to numericalize the text/field
    ...
    # get vocab vector for numeric thres.
    pretrained_embeddings = pretrained_vectors.vectors
    pretrained_embeddings = torch.cat(
        (torch.zeros(2, pretrained_embeddings.shape[1]), pretrained_embeddings))
    ...
    all_sentences = data[['pros', 'cons']].applymap(lambda x: nltk.word_tokenize(
        preprocess_transformers(x, stemmer, en_stop, full_process=False)))
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        'prajjwal1/bert-tiny') if True else AutoTokenizer.from_pretrained("bert-base-uncased")

    # preprocess the columns in order to tokenize them
    data['headline'] = data['headline'].progress_apply(
        lambda x: preprocess_transformers(x, stemmer, en_stop, full_process=False))
    data['pros'] = data['pros'].progress_apply(
        lambda x: preprocess_transformers(x, stemmer, en_stop, full_process=False))
    data['cons'] = data['cons'].progress_apply(
        lambda x: preprocess_transformers(x, stemmer, en_stop, full_process=False))

    # numeric tokenization
    data['headline'] = data['headline'].progress_apply(
        lambda x: ' '.join(phraser[word_tokenize(x)]))
    data['pros'] = data['pros'].progress_apply(
        lambda x: ' '.join(phraser[word_tokenize(x)]))
    data['cons'] = data['cons'].progress_apply(
        lambda x: ' '.join(phraser[word_tokenize(x)]))
    ...
    data['tokenized_headline'] = data['headline'].progress_apply(
        lambda x: tokenize_pad_numericalize(x, stoi, tok, pad, max_length=20))
    data['tokenized_pros'] = data['pros'].progress_apply(
        lambda x: tokenize_pad_numericalize(x, stoi, tok, pad, max_length=80))
    data['tokenized_cons'] = data['cons'].progress_apply(
        lambda x: tokenize_pad_numericalize(x, stoi, tok, pad, max_length=100))

    # Preprocess text for transformers
    data['tokenized_headline'] = data.apply(lambda row: str_replace_tokens(
        row['headline'], row['tokenized_headline']), axis=1)
    data['tokenized_pros'] = data.apply(lambda row: str_replace_tokens(
        row['pros'], row['tokenized_pros']), axis=1)
    data['tokenized_cons'] = data.apply(lambda row: str_replace_tokens(
        row['cons'], row['tokenized_cons']), axis=1)
...
def tokenize_pad_numericalize(entry, vocab_stoi, tok, pad=True, max_length=100) -> list[int]:
    ...
    if pad:
        text = [vocab_stoi[token] if token in vocab_stoi else vocab_stoi['']
                for token in tok.tokenize(entry)]
        padded_text = None
        l = len(text)
        if l < max_length:
            padded_text = text + [vocab_stoi['']
